...
type Filler[K Key[KH], KH KeyHash] interface {
	...
	WriteCacheData(key K, sink io.Writer) (written int64, err error)
}

// FillerFunc is used by GetOrPut and GetReaderOrPut to atomically insert cache entries.
// Write the content for key into sink and return the number of written bytes.
// Errors are passed through to GetOrPut or GetReaderOrPut.
type FillerFunc[K Key[KH], KH KeyHash] func(key K, sink io.Writer) (written int64, err error)

func (f FillerFunc[K, KH]) WriteCacheData(key K, sink io.Writer) (written int64, err error) {
	return f(key, sink)
}
...
type Cache[K Key[KH], KH KeyHash] interface {
	...
	Has(key K) (info *EntryInfo, err error)
	...
	GetReader(key K) (reader io.ReadSeekCloser, info *EntryInfo, err error)
	...
	GetOrPut(key K, ttl time.Duration, filler Filler[K, KH]) (data []byte, info *EntryInfo, hit bool, err error)
	...
	GetReaderOrPut(key K, ttl time.Duration, filler Filler[K, KH]) (reader io.ReadSeekCloser, info *EntryInfo, hit bool, err error)

	// Delete removes the cache entry at key.
	// If no cache entry exists at key nothing happens.
	Delete(key K) (info *EntryInfo, err error)
