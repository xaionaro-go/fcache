...
type Filler interface {
	...
	WriteCacheData(key Key, sink io.Writer) (written int64, err error)
}

// FillerFunc is used by GetOrPut and GetReaderOrPut to atomically insert cache entries.
// Write the content for key into sink and return the number of written bytes.
// Errors are passed through to GetOrPut or GetReaderOrPut.
type FillerFunc func(key Key, sink io.Writer) (written int64, err error)

func (f FillerFunc) WriteCacheData(key Key, sink io.Writer) (written int64, err error) {
	return f(key, sink)
}
...
type Cache interface {
	...
	Has(key Key) (info *EntryInfo, err error)
	...
	GetReader(key Key) (reader io.ReadSeekCloser, info *EntryInfo, err error)
	...
	GetOrPut(key Key, ttl time.Duration, filler Filler) (data []byte, info *EntryInfo, hit bool, err error)
	...
	GetReaderOrPut(key Key, ttl time.Duration, filler Filler) (reader io.ReadSeekCloser, info *EntryInfo, hit bool, err error)

	// Delete removes the cache entry at key.
	// If no cache entry exists at key nothing happens.
	Delete(key Key) (info *EntryInfo, err error)
